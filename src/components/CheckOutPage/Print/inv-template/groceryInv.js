...
export class GroceryInvoice extends Component {
    ...
    getProductOurPrice(product) {
        let finalprice = product?.finalprice

        if (product?.item_applied_disc_amt_ttl > 0) {
            finalprice -= product?.item_applied_disc_amt_ttl;
        }
    ...
    render() {
        ...
        // this.getFromSession('show_unit_invoice')
                        ...
                        <tbody>
                            <tr className="inv-table-row" style={{ borderBottom: "2px solid #333" }}>
                                <th className="inv-table-data inv-table-data-left" colSpan={4}>
                                    <Typography className="custom-font" variant="subtitle2" component="b"> <strong>Description</strong></Typography>
                                </th>
                                ...
                                <th className="inv-table-head inv-table-data-right" colSpan={2}>
                                    <Typography className="custom-font" variant="subtitle2" component="b"> <strong>price</strong></Typography>
                                </th>
                                <th className="inv-table-head inv-table-data-right" colSpan={2}>
                                ...
                                <tr key={product.id} className="inv-table-row">
                                    ...
                                    <td className="inv-table-data inv-table-data-right" colSpan={2}>
                                        <Typography className="custom-font" variant="body2" component="b">
                                            {/* {CartHelper.getCurrencyFormatted(Number(product.finalprice).toFixed(2))} */}
                                            {(Number(this.getProductOurPrice(product)).toFixed(2))}

                                        </Typography>
                                    </td>
                                    <td className="inv-table-data inv-table-data-right" colSpan={2}>
                                        <Typography className="custom-font" variant="body2" component="b">{Number(product.qty).toFixed(quantity_decimal)}</Typography> <br />
                                        {showUnit &&
                                            <Typography className="custom-font" variant="body2" component="b">{product.unit}</Typography>
                                        }
                                    </td>
                                    <td className="inv-table-data inv-table-data-right" colSpan={2}>
                                        <Typography className="custom-font" variant="body2" component="b">{(Number(this.getProductOurPrice(product) * product.qty).toFixed(2))}</Typography>
                                    </td>
                                </tr>
                            ))}
                            ...
                            {Number(billingData.sales_data.delivery_charge) > 0 && <tr className="inv-table-row">
                                <td colSpan={4}></td>
                                <td className="inv-table-data align-right" colSpan={4}>
                                    <Typography className="custom-font" variant="subtitle2" component="b"> <strong>Delivery Charge</strong>
                                    </Typography>
                                </td>
                                <td className="inv-table-data align-right" colSpan={4}>
                            ...
                            {Number(totalSavedAmt) > 0 ?
                                <tr className="inv-table-row">
                                    <td className="align-left" colSpan={12}>
                                        <Box pt={2}>
                                            <Typography style={{ fontWeight: "bold" }} ><strong>You saved:
