...
class searchByCategory extends React.Component {
  ...
  getSuggestions = value => {
    ...
    // return inputLength === 0 ? [] : productData.filter(product =>
    //     product.cat_name.toLowerCase().slice(0, inputLength) === inputValue
    // );

    return productData.filter(product => regex.test(product.name) || regex.test(product.cat_name));
  };
  ...
  addThisProductToCArt = (product) => {
    ...
    if (this.props.cartProduct.length !== 0) {
      ...
      this.props.cartProduct.forEach(prod => {
        ...
        if (prod.barcode === nextProduct.barcode) {
          this.props.updateQty(nextProduct);
          return isExist = 1;
        }
      });

      if (!isExist) {
        this.props.AddToCart(nextProduct);
      }
...
export default connect(mapStateToProps, { AddToCart, updateQty })(searchByCategory)
